...
try:
    ...
    from reporting.report_builder import build_forensic_report
...
class EvidenceEngine:
    ...
    def _pre_check_analysis(self, file_bytes: bytes, filename: str):
        ...
        if ext in {'.mp4', '.mov', '.avi'}:
            with tempfile.TemporaryDirectory() as tmp_dir:
                tmp_path = os.path.join(tmp_dir, f'precheck{ext}')
                ...
                if frame_count < 5:
                    raise ValueError(
                        f"Evidence file contains too few frames ({frame_count}) for reliable AI analysis."
                    )
                if duration <= 0:
    ...
    def run_analysis(
        ...
        def update_prog(progress, message):
            if progress_callback:
                progress_callback(progress, message)
            logger.info("Engine: %s%% - %s", progress, message)
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = os.path.join(tmp_dir, f'evidence{ext}')
            with open(tmp_path, 'wb') as tmp:
                tmp.write(file_bytes)
